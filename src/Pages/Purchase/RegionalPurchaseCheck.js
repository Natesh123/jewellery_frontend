...
import {
    ...
    getAllQuotationsRegionaForPurchases,
    updatePurchaseRegional
...
const RegionalPurchaseCheck = () => {
    ...
    const handleSubmit = async () => {
        ...
        await Swal.fire({
            title: 'Sent',
            text: 'Your purchase sent to accounts',
            icon: 'success',
            confirmButtonText: 'OK'
        });
    ...
    const handleExportPDF = async () => {
        try {
            ...
            // Generate PDF
            const pdfDoc = generateRegionalPurchasePDF("REGIONAL PURCHASE REPORT", exportData, exportFilters, metals);
    ...
    const updateRegionalStatus = async (purchase, status) => {
        ...
        try {
            ...
            setPurchases(prevPurchases =>
                prevPurchases.map(p =>
                    p.id === purchase.id ? { ...p, regional_status: status } : p
                )
            );

            // Update on the server
            const response = await updatePurchaseRegional(purchase.id, {
            ...
            if (response) {
                Swal.fire({
                    ...
                    text: `Purchase ${purchase.purchase_id || purchase.quotation_id} marked as sent to accounts`,
                    timer: 2000,
                    showConfirmButton: false
                });
            ...
            setPurchases(prevPurchases =>
                prevPurchases.map(p =>
                    p.id === purchase.id ? { ...p, regional_status: purchase.regional_status } : p
                )
            );
        }
    };
    ...
    const regionalStatusSwitch = (record) => (
        ...
        <Spin spinning={loading}>
            <Checkbox
                disabled={record.regional_status === "1"}
                checked={record.regional_status === "1"}
                onChange={(e) => {
    ...
    useEffect(() => {
        console.log("Purchases updated:", purchases.map(p => ({
            ...
            purchase_id: p.purchase_id,
            quotation_id: p.quotation_id,
            regional_status: p.regional_status
        })));
    }, [purchases]);
    // Load initial data
    useEffect(() => {
    ...
    const fetchInitialData = async () => {
        try {
            ...
            // Fetch purchases
            const purchasesResponse = await getAllQuotationsRegionaForPurchases()
    ...
    const purchaseColumns = [
        ...
        },
        {
            ...
            render: (_, record) => {
                ...
                if (record.accounts_status === null || record.accounts_status === undefined || record.accounts_status === 0) {
                    paymentText = 'Not Approved';
                    tagColor = 'green';
                } else {
                    paymentText = 'Approved';
                    tagColor = 'orange';
                }

                return <Tag color={tagColor}>{paymentText}</Tag>;
            }
        },
...
export default RegionalPurchaseCheck;