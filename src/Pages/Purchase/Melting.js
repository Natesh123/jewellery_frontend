...
import {
    ...
    createMeltProducts, getMeltProducts, updateMeltProduct,
    updateMeltDetails,
    updateMeltWages,
    getAllSmith
...
const MeltingStatus = () => {
    ...
    const [smithOptions, setSmithOptions] = useState([]);
    ...
    const fetchAllSmith = async () => {
        try {
            const response = await getAllSmith();
            const smithData = response?.data || [];
            console.log(smithData)
            setSmithOptions(smithData);
        } catch (error) {
            message.error('Failed to fetch metal options');
            console.error('Error fetching metals:', error);
        }
    ...
    const fetchMeltProducts = async (page = 1, pageSize = 10, filters = {}) => {
        try {
            ...
            const response = await getMeltProducts({
                page: page,
                limit: pageSize,
            ...
            >
                ...
                >
                    <Form.Item
                        label="Smith Name"
                        name="smith_name"
                        rules={[{ required: true, message: 'Please select smith name' }]}
                    >
                        <Select
                        ...
                        >
                            {smithOptions.map(item => (
                                <Select.Option
                                    key={item.id}
                                    value={`${item.head_name} - ${item.account_code}`}   // or item.account_code if needed
                                    label={`${item.head_name} - ${item.account_code}`}
                                >
                                    {item.head_name} ({item.account_code})
                                </Select.Option>
                            ))}
                        </Select>
                    </Form.Item>
                    ...
                    >
                        <div style={{ padding: '8px', backgroundColor: '#f5f5f5', borderRadius: '4px' }}>
                            <Text type="secondary">
                                Product ID: <Text strong>#AMAMELT{product?.id}</Text>
                            </Text>
                            <br />
                            <Text type="secondary">
        ...
        <div style={{ padding: '24px', backgroundColor: roots.background.default, minHeight: '100vh' }}>
            ...
            <Card style={{ marginBottom: 16 }}>
                <Row gutter={[16, 16]} justify="center" align="middle">
                    <Text style={{ fontSize: 24 }}>Melting (Smith Assign)</Text>
                </Row>
