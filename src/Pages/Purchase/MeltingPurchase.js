...
import { createMeltingPurchase, getAllkMeltingPurchases, getMeltingPurchase, updatePurchaseAccountsStatus } from '../../api/services/MeltingPurchaseService';
...
const MeltingPurchase = () => {
    ...
    const handleSubmit = async () => {
        ...
        await Swal.fire({
            title: 'Collected',
            text: 'Your purchase collected from Accounts ',
            icon: 'success',
            confirmButtonText: 'OK'
        });
    ...
    const handleExportPDF = async () => {
        try {
            ...
            // Fetch all data for export (you might need a separate endpoint for this)
            const exportResponse = await getAllkMeltingPurchases({
                limit: 10000, // Large limit to get all data, or create a separate export endpoint
                search: purchaseFilters.search,
                metal: purchaseFilters.metal,
                status: purchaseFilters.status
            });

            const exportData = exportResponse.purchases || [];
            ...
            // Generate PDF
            const pdfDoc = generateRegionalPurchasePDF("MELTING PURCHASE REPORT", exportData, exportFilters, metals);
            ...
            const filename = `melting-purchases-${dateSuffix}.pdf`;
    ...
    const updateRegionalStatus = async (purchase, status) => {
        ...
        try {
            ...
            setPurchases(prevPurchases =>
                prevPurchases.map(p =>
                    p.id === purchase.id ? { ...p, collect_accounts_status: status } : p
                )
            );            

            // Update on the server
            const response = await updatePurchaseAccountsStatus(purchase.id, {
                collect_accounts_status: status
            });
            ...
            if (response) {
                Swal.fire({
                    ...
                    text: `Purchase ${purchase.purchase_id || purchase.quotation_id} collected from Accounts`,
                    timer: 2000,
                    showConfirmButton: false
                });
            ...
            setPurchases(prevPurchases =>
                prevPurchases.map(p =>
                    p.id === purchase.id ? { ...p, collect_accounts_status: purchase.collect_accounts_status } : p
                )
            );
        }
    };
    ...
    const regionalStatusSwitch = (record) => (
        ...
        <Spin spinning={loading}>
            <Checkbox
                checked={record.collect_accounts_status === 1}
                disabled={record.collect_accounts_status === 1} // ✅ disable if already collected
                onChange={(e) => {
    ...
    useEffect(() => {
        console.log("Purchases updated:", purchases.map(p => ({
            ...
            purchase_id: p.purchase_id,
            quotation_id: p.quotation_id,
            collect_accounts_status: p.collect_accounts_status
        })));
    }, [purchases]);
    // Load initial data
    useEffect(() => {
    ...
    const fetchInitialData = async (page = 1, pageSize = 10) => {
        try {
            ...
            const purchasesResponse = await getAllkMeltingPurchases({
                page: page,
                limit: pageSize,
                search: purchaseFilters.search,
                metal: purchaseFilters.metal,
    ...
    // Update applyPurchaseFilters to handle API calls with pagination
    ...
    // Update useEffect for purchase filters
    useEffect(() => {
        applyPurchaseFilters(1); // Reset to page 1 when filters change
    }, [purchaseFilters]);
    ...
    const purchaseColumns = [
        ...
        {
            title: 'Collected From Accounts',
            key: 'collect_accounts_status',
            width: 200,
...
export default MeltingPurchase;