...
const AccountsApproval = () => {
  ...
  const fetchPledges = async (page = 1, pageSize = 10) => {
    try {
      setLoading(true);
      const response = await pledgeService.getAllAccountsApprovalPledges(page, pageSize);
  ...
  const handleAssign = async () => {
    ...
    try {
      setLoading(true);
      await pledgeService.assignAccountsApproval(currentPledgeId, { approval_accounts_status: selectedExecutive, user_id: localStorage.getItem("userId") });
