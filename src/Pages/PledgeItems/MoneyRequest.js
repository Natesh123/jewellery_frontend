...
const MoneyRequest = () => {
  ...
  const fetchPledges = async (page = 1, pageSize = 10) => {
    try {
      setLoading(true);
      const response = await pledgeService.getAllMoneyRequestPledges(page, pageSize);
        ...
        async (position) => {
          try {
            ...
            if (result.isConfirmed) {
              ...
              const updateData = {
                money_request_lat: latitude,
                money_requet_lng: longitude,
                money_rquest_status: selectedExecutive,
