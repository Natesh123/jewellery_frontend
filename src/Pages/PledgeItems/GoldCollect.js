...
const GoldCollect = () => {
  ...
  const fetchPledges = async (page = 1, pageSize = 10) => {
    try {
      setLoading(true);
      const response = await pledgeService.getAllGoldCollectPledges(page, pageSize);
        ...
        async (position) => {
          try {
            ...
            if (result.isConfirmed) {
              ...
              const updateData = {
                gold_collect_lat: latitude,
                gold_collect_lng: longitude,
                gold_collect_status: selectedExecutive,
                user_id: localStorage.getItem("userId") || "0"
              };

              // Ensure the pledge ID is correctly passed
              await pledgeService.updategoldcollectRequest(currentPledgeId, updateData);
  ...
  const pledgeColumns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const { color, text } = statusMap[record.gold_collect_status] || { color: 'blue', text: 'Pending' };
  ...
  return (
    <div style={{ padding: '24px', backgroundColor: roots.background.default, minHeight: '100vh' }}>
      ...
      <Modal
        title="Have u collected the gold"
