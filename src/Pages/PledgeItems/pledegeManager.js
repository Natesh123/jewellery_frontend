...
const PledgeManager = () => {
  ...
  const fetchPledges = async (page = 1, pageSize = 10) => {
    try {
      setLoading(true);
      const response = await pledgeService.getAllManagerPledges(page, pageSize);
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        // Collect all approval IDs
        const approvalIds = pledges.map(item => item.approval).filter(Boolean);
  
        // Fetch users in bulk (better) or individually
        const userResponses = await Promise.all(
          approvalIds.map(id => getUserById(id))
        );
  ...
  const handleAssign = async () => {
    ...
    try {
      setLoading(true);
      await pledgeService.updateSalesExecutive(currentPledgeId, { reference: selectedExecutive });
  ...
  const pledgeColumns = [
    ...
    {
      title: 'Sales Executive Status',
      key: 'status',
      width: 120,
      render: (_, record) => {
        const isAssigned = record.approval !== null;
        return (
    ...
    },    
    {
      ...
      width: 180,
      render: (_, record) => {
        const approvalId = Number(record.approval); 
        const user = executives.find(item => item.user_id === approvalId);
    
        return (
          <Tag color={user ? "green" : "red"}>
            {user ? `${user.first_name} ${user.last_name}` : "Still Not Assigned"}
          </Tag>
        );
      }
    },
  ...
  return (
    <div style={{ padding: '24px', backgroundColor: roots.background.default, minHeight: '100vh' }}>
      ...
      >
        ...
        >
          {executives.map((exec) => (
            <Select.Option key={exec.user_id} value={exec.user_id}>
