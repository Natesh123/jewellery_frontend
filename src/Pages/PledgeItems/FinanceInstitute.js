...
const FinanceInstitute = () => {
    ...
    const fetchPledges = async (page = 1, pageSize = 10) => {
        try {
            setLoading(true);
            const response = await pledgeService.getAllFinanceInstitutePledges(page, pageSize);
    ...
    const handleAssign = async () => {
        if (!selectedExecutive || !currentPledgeId) {
            message.warning('Please select a status and ensure pledge is selected');
            return;
        }
        ...
        try {
            ...
            navigator.geolocation.getCurrentPosition(
                async (position) => {
                    try {
                        ...
                        if (result.isConfirmed) {
                            // Make sure to use the correct property names that your API expects
                            const updateData = {
                                finance_lat: latitude,
                                finance_lng: longitude,
                                finance_status: selectedExecutive,
                                user_id: localStorage.getItem("userId") || "0"
                            };

                            // Ensure the pledge ID is correctly passed
                            await pledgeService.updateFinanceInstituteRequest(currentPledgeId, updateData);
                        ...
                        Swal.fire("Error", "Failed to update pledge", "error");
                    } finally {
                ...
                (error) => {
                    setLoading(false);
                    Swal.fire("Location Error", "Please enable location services to proceed", "error");
            ...
            setLoading(false);
            message.error('Failed to update pledge status');
        }
    };
    ...
    const pledgeColumns = [
        ...
        {
            title: 'Approved Amounts',
            key: 'amounts',
            width: 180,
            render: (_, record) => (
                <div>
                    <div>
                        ₹ {record.accounts_amount != null ? new Intl.NumberFormat('en-IN').format(record.accounts_amount) : '—'}
                    </div>
                </div>
            )
        },
        {
            ...
            render: (_, record) => (
                <div>
                    <div>
                        {record.pledge_amount && record.accounts_amount
                            ? `₹ ${new Intl.NumberFormat('en-IN').format(record.pledge_amount - record.accounts_amount)}`
                            : '—'}
                    </div>
        ...
        },
        {
            ...
            render: (_, record) => {
                console.log(record)
                const statusMap = {
                    "1": { color: 'green', text: 'Issued' },
                    "2": { color: 'orange', text: 'Progress' },
                    "3": { color: 'red', text: 'Not Issued' }
                };

                const { color, text } = statusMap[record.finance_status] || { color: 'blue', text: 'Pending' };
            ...
            render: (_, record) => (
                <Space>
                    ...
                    {/* <Popconfirm
                        ...
                        onConfirm={() => handleDeletePledge(record.key)}
                        okText="Yes"
                        cancelText="No"
                    >
  ...
  `;
    return (
        <div style={{ padding: '24px', backgroundColor: roots.background.default, minHeight: '100vh' }}>
            ...
            <Modal
                title="Money Given to Finance Institute"
...
export default FinanceInstitute;
