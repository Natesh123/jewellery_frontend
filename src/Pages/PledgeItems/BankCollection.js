...
const BankCollection = () => {
    ...
    const fetchPledges = async (page = 1, pageSize = 10) => {
        try {
            setLoading(true);
            const response = await pledgeService.getAllBankCollectionPledges(page, pageSize);
            ...
            async (position) => {
              try {
                ...
                if (result.isConfirmed) {
                  ...
                  const updateData = {
                    bank_collection_lat: latitude,
                    bank_collection_lng: longitude,
                    bank_collection_status: selectedExecutive,
                    user_id: localStorage.getItem("userId") || "0"
                  };
    
                  // Ensure the pledge ID is correctly passed
                  await pledgeService.updateBankCollectionRequest(currentPledgeId, updateData);
    ...
    const pledgeColumns = [
        ...
        },
        {
            ...
            render: (_, record) => {
                console.log(record)
                const statusMap = {
                    "1": { color: 'green', text: 'Collected' },
                    "2": { color: 'orange', text: 'Progress' },
                    "3": { color: 'red', text: 'Rejected' }
                };

                const { color, text } = statusMap[record.bank_collection_status] || { color: 'blue', text: 'Pending' };
  ...
  `;
    return (
        <div style={{ padding: '24px', backgroundColor: roots.background.default, minHeight: '100vh' }}>
            ...
            <Modal
                title="Money Collected From Bank"
