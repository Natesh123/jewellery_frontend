...
const Accounts = () => {
  ...
  const fetchPledges = async (page = 1, pageSize = 10) => {
    try {
      setLoading(true);
      const response = await pledgeService.getAllAccountsPledges(page, pageSize);
  ...
  const handleAssign = async () => {
    ...
    try {
      setLoading(true);
      await pledgeService.updateAccountRequest(
        currentPledgeId,
        {
          accounts_status: selectedExecutive,
  ...
  const pledgeColumns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        console.log(record)
        const statusMap = {
          "1": { color: 'green', text: 'Cheque Send to Manager' },
          "2": { color: 'orange', text: ' Cash Send to Manager' }
        };
      ...
      >
        ...
        >
          <Select.Option key={'1'} value={'1'}>
            Cheque Send to Manager
          </Select.Option>
          <Select.Option key={'2'} value={'2'}>
            Cash Send to Manager
          </Select.Option>
        </Select>
      </Modal>
