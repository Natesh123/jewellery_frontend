...
const PledgeManageApproval = () => {
    ...
    const fetchPledges = async (page = 1, pageSize = 10) => {
        try {
            setLoading(true);
            const response = await pledgeService.getAllOfficePledges(page, pageSize);
            ...
            const transformedPledges = pledgesData.map(pledge => {
                if (pledge.manage_approval_status === '1') {
                  return {
                    key: pledge.id,
                    ...
                    gross_weight: pledge.product_details?.reduce(
                      (sum, item) => sum + (parseFloat(item.gross_weight) || 0),
                      0
                    ),
                    net_weight: pledge.product_details?.reduce(
                      (sum, item) => sum + (parseFloat(item.net_weight) || 0),
                      0
                    ),
                    amount: pledge.product_details?.reduce(
                ...
                // if condition not matched, return null
                return null;
            ...
            async (position) => {
              try {
                ...
                if (result.isConfirmed) {
                  ...
                  const updateData = {
                    manage_approval_status: selectedExecutive,
                  };
    
                  // Ensure the pledge ID is correctly passed
                  await pledgeService.updateManageApprovalRequest(currentPledgeId, updateData);
    ...
    const pledgeColumns = [
        ...
        },
        {
            ...
            render: (_, record) => {
                ...
                const { color, text } = statusMap[record.manage_approval_status] || { color: 'blue', text: 'Pending' };
...
export default PledgeManageApproval;
