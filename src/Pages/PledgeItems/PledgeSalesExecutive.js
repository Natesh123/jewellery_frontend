...
const PledgeSalesExecutive = () => {
  ...
  const fetchPledges = async (page = 1, pageSize = 10) => {
    try {
      ...
      const transformedPledges = pledgesData.map(pledge => ({
        ...
        approval: pledge.accounts_amount,
        bill: pledge.bill,
  ...
  const handleAssign = async () => {
    ...
    try {
      setLoading(true);
      await pledgeService.updateExecutive(currentPledgeId, { approval: selectedExecutive, accounts_amount: amount });
  ...
  return (
    <div style={{ padding: '24px', backgroundColor: roots.background.default, minHeight: '100vh' }}>
      ...
      >
        <Select
          style={{ width: '100%' }}
          placeholder="Money Request send to Accounts"
          loading={loading}
...
export default PledgeSalesExecutive;
