...
const Collections = () => {
  ...
  const fetchPledges = async (page = 1, pageSize = 10) => {
    try {
      setLoading(true);
      const response = await pledgeService.getAllCollectionPledges(page, pageSize);
      ...
      const transformedPledges = pledgesData.map(pledge => ({
        ...
        collection_type: pledge.collection_type,
        bill: pledge.bill,
  ...
  const handleAssign = async () => {
    ...
    try {
      setLoading(true);
      await pledgeService.updateCollectionRequest(currentPledgeId, { collection_type: selectedExecutive });
      message.success('Collection Updated successfully!');
      setIsAssignModalVisible(false);
      setSelectedExecutive(null);
      fetchPledges()
      // You might want to refresh your pledges data here
    } catch (error) {
  ...
  const pledgeColumns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        console.log(record)
        const statusMap = {
          "1": { color: 'green', text: 'Cheque Request' },
          "2": { color: 'orange', text: ' Cash Request' }
        };

        const { color, text } = statusMap[record.accounts_status] || { color: 'blue', text: 'Pending' };
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const statusMap = {
          ...
          "2": { color: 'orange', text: 'Decline' }
        };

        const { color, text } = statusMap[record.collection_type] || { color: 'blue', text: 'Pending' };
  ...
  return (
    <div style={{ padding: '24px', backgroundColor: roots.background.default, minHeight: '100vh' }}>
      ...
      <Modal
        title="Cheque or Cash Receive From Accounts"
